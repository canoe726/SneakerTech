...
$(function() {
    ...
    $('.d-day-timer').html(show_Timer());
});

function draw_btn_hover() {
    document.getElementById('origin-page').innerHTML='Enter Draw';
}

function draw_btn_out(text) {
    document.getElementById('origin-page').innerHTML=text;
}

function show_Timer() {
    var html = ``;

    var d_day = new Date("2020-06-26T10:00");
    var cur_day = new Date();

    d_day = momnet();

    console.log(d_day);
    console.log(cur_day.format(cur_day, "YYYY-MM-DD HH:mm"));

    console.log(moment.duration(d_day.diff(cur_day)).asDays());
        ...
        for(var idx = start_idx; idx < start_idx + item; idx ++) {
            ...
            <div id="release-card" class="card">
                ...
                <a target="_blank" rel="noopener noreferrer" href="${site_page[idx].url}">
                    <img src="${site_page[idx].img}" class="card-img-top">
                </a>
                <div class="card-body" style="text-align: center;">
